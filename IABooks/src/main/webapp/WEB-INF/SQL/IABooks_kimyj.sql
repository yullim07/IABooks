...
create sequence seq_tbl_writer
start with 4000
increment by 1
nomaxvalue
nominvalue
...
VALUES (seq_tbl_writer.nextval, '테스트', '테스트')
commit
seq_tbl_writer.nextval
1. INSERT INTO 테이블이름(필드이름1, 필드이름2, 필드이름3, ...)
   ...
   VALUES (데이터값1, 데이터값2, 데이터값3, ...)
    ...
    select sum(MILEAGEINFO)from TBL_MILEAGE  where FK_USERID = 'admin';
    
    
    
update tbl_product set  PRO_SOLDOUT = 0
where PRO_QTY = 0;

--프로덕트 : 번호, 이름, 사진, 
-- 오더디테일 : 
--
select  PK_PRO_NUM, PRO_NAME, PRO_IMGFILE_NAME, CATE_NAME, PK_ODR_SEQNUM, FK_ODRCODE, CK_ODR_TOTALQTY, ODR_PRICE, CK_DELIVERSTATUS, DELIVERNAME, FK_USERID, ODR_TOTALPRICE, ODR_DATE, RV_NAME, RV_ZIPCODE, RV_ADDR, RV_PHONE, RV_EMAIL, DEL_MSG, PRO_SALEPRICE
from tbl_product 
join TBL_CATEGORY
on FK_CATE_NUM = PK_CATE_NUM
...
update tbl_orderdetail set ck_deliverstatus = 3
 where pk_odr_seqnum = 22
 
 
 select pk_userid, mname, address, detailaddress, extraaddress, uq_phone, uq_email 
from tbl_member 
where pk_userid = 'admin';


update tbl_orderdetail set ck_deliverstatus = 4
where pk_odr_seqnum = 22  