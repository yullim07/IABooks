...
public class AdminBoardDeleteSelectAction extends AbstractController {
		...
		if(super.checkLogin(request)) { //로그인했으면 true
			...
			if("POST".equalsIgnoreCase(method)) {//포스트 방식이라며는 
	            ...
	            for(int i=0; i<revCnt; i++) {
	                ...
	                m = tbdao.revDeleteSelect(paraMap);
	            }
	            ...
	            for(int i=0; i<qnaCnt; i++) {
	                ...
	                n = tbdao.qnaDeleteSelect(paraMap);
	            }
	            ...
	            if( "".equalsIgnoreCase(qnaNoStr) && !"".equalsIgnoreCase(revNoStr)) {
	            	...
	            	if( n == 1) {
	            		// 상품문의게시판 게시글이 삭제가 되었다면
	            		result = 1;
	            	}
	            }
	            else if( n == 1 && m == 1) {
	            	// 두 게시판 모두 선택하였고 삭제되었다면
	            	result = 1;
	            }
	            
				JSONObject jsonObj = new JSONObject(); 
				jsonObj.put("adminBoardDeleteSelect", result); 
