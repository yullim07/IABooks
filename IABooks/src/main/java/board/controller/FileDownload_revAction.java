...
public class FileDownload_revAction extends AbstractController {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		//	System.out.println("=== 첨부되어지는 이미지 파일이 올라가는 절대경로 uploadFileDir ==> " + uploadFileDir);
			...
			Map<String,String> map = bdao.getRevImgFileName(pk_rnum);
			
			// >>> File 객체 생성하기 <<< //
		//	String filePath = uploadFileDir +"\\" + map.get("prdmanual_systemFileName");
			String filePath = uploadFileDir + File.separator + map.get("rev_file_system_name");
	   ...
	   //   System.out.println("~~~~ 확인용 mimeType => " + mimeType);
	        ...
	        String rev_file_original_name = map.get("rev_file_original_name");
	     // map.get("qna_file_original_name")이 웹클라이언트의 웹브라우저에서 파일을 업로드 할때 올리는 제품설명서 파일명임.
	        ...
	        if (header.contains("Edge")){
	            downloadFileName = URLEncoder.encode(rev_file_original_name, "UTF-8").replaceAll("\\+", "%20");
	             response.setHeader("Content-Disposition", "attachment;filename=" + downloadFileName);
	        } else if (header.contains("MSIE") || header.contains("Trident")) { // IE 11버전부터는 Trident로 변경됨.
	             downloadFileName = URLEncoder.encode(rev_file_original_name, "UTF-8").replaceAll("\\+", "%20");
	             response.setHeader("Content-Disposition", "attachment;filename=" + downloadFileName);
	        } else if (header.contains("Chrome")) {
	            downloadFileName = new String(rev_file_original_name.getBytes("UTF-8"), "ISO-8859-1");
	             response.setHeader("Content-Disposition", "attachment; filename=" + downloadFileName);
