...
@WebServlet(
		...
		urlPatterns = { "*.up" }, 
		initParams = { 
				@WebInitParam(name = "propertyConfig", value = "C:/NCS/workspace(jsp)/IABooks/src/main/webapp/WEB-INF/Command.properties", description = "*.up 에 대한 클래스의 매핑파일")
		})
	...
	//	System.out.println("~~~ 확인용 props => " + props); // @WebInitParam의 value값이 나와야 한다.
		...
		try {
			...
			*/
			...
			System.out.println(">>> C:/NCS/workspace(jsp)/IABooks/src/main/webapp/WEB-INF/Command.properties 파일이 없습니다.");
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
